...
@Component("filedown")     //설정파일에 자동 등록 처리함 filedown을 id명으로
public class FileDownloadView extends AbstractView {
    //스프링에서는 뷰 클래스를 만들려면
    //스프링이 제공하는 AbstractView 를 상속 받아서
    //오버라이딩 한 메소드 안에 기능을 구현하도록 되어 있음

    @Override
    protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        // 파일 다운로드 처리용 코드 구현함
        // 컨트롤러 메소드에서 리턴 한 모델의 값(파일 객체)을 추출해서
        ...
        //저장 폴더에서 renameFile을 read() 해서 response 로 write() | print() 함 => 스프링에서 제공
